...
pub const CPU = struct {
    ...
    pub inline fn tick(self: *Self) void {
        ...
        switch(instr.opcode) {
            ...
            .ADD    => self.instrAdd(instr.operand, 0),
            .ADD_f  => self.instrAdd(instr.operand, 1),
            .ADDI   => self.instrAddi(instr.operand, 0),
            .ADDI_f => self.instrAddi(instr.operand, 1),
            .NAND   => self.instrNand(instr.operand, 0),
            .NAND_f => self.instrNand(instr.operand, 1),
            .LD     => self.instrLd(instr.operand, 0),
            .LD_f   => self.instrLd(instr.operand, 1),
            .ST     => self.instrSt(instr.operand),
            .B      => self.instrB(instr.operand),
            .B_h    => self.instrB(instr.operand)
        }
    }
    ...
    pub fn getNextInstruction(self: Self, buffer: *std.ArrayList(u8)) Result {
        ...
        const result = switch(instr.opcode) {
            ...
            .ADD     => writer.print("add r{d}",    .{instr.operand}),
            .ADD_f   => writer.print("add.f r{d}",  .{instr.operand}),
            .ADDI    => writer.print("addi {d}",    .{instr.operand}),
            .ADDI_f  => writer.print("addi.f {d}",  .{instr.operand}),
            .NAND    => writer.print("nand r{d}",   .{instr.operand}),
            .NAND_f  => writer.print("nand.f r{d}", .{instr.operand}),
            .LD      => writer.print("ld r{d}",     .{instr.operand}),
            .LD_f    => writer.print("ld.f r{d}",   .{instr.operand}),   
            .ST      => writer.print("st r{d}",     .{instr.operand}),   
            .B, .B_h => writer.print("b {s}", .{switch(instr.operand) {
                0b0000 => "false",
                0b0001 => "true",
                0b0010 => "!a",
                0b0011 => "a",
        ...
        result catch return Result.errStatic("Failed to allocate buffer");
    ...
    inline fn instrLda(self: *Self, operand: u4, flag_update: u1) void {
